...
{#if auth.isAuthenticated}
	<div>
		<h1 class="text-2xl font-bold mb-4">Welkom terug, {auth.user?.name}!</h1>
					...
					<div class="space-y-3">
						{#each upcomingQuery.data as game}
							<a href="/games/{game.id}" class="block p-3 rounded-lg hover:preset-tonal-surface transition-colors -mx-1">
								...
								<div class="text-xs text-surface-400 mt-1">{formatDate(game.dateTime)}</div>
							</a>
						{/each}
					</div>
				{/if}
...
{:else}
	<div class="text-center py-16">
		...
		<p class="text-lg text-surface-400 mb-8">Welkom bij de Noordpool voetbal app.</p>
		<a
			href="/auth/login"

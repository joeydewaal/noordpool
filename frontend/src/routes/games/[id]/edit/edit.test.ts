...
describe('Game edit page', () => {
	...
	it('populates the datetime-local input with a local-time value (no Z suffix)', async () => {
		...
		const input = (await screen.findByLabelText('Datum & tijd')) as HTMLInputElement;
		...
		expect(new Date(input.value).toISOString()).toBe('2026-06-15T18:00:00.000Z');
	});
	...
	it('submits dateTime as a UTC ISO string regardless of local TZ', async () => {
		...
		expect(new Date(payload.dateTime).toISOString()).toBe('2026-06-15T18:00:00.000Z');
	});

...
<div class="max-w-md mx-auto mt-8">
  ...
  <p class="text-surface-400 text-sm mb-6">
    Ben jij een van de spelers hieronder? Koppel je account om statistieken bij
    te houden.
  </p>
  ...
  {:else if playerMatches.length > 0}
    <div class="card preset-tonal-warning p-4 mb-4 space-y-3">
      <p class="text-sm font-medium">
        We vonden de volgende spelers. Ben jij dit?
      </p>
      {#each playerMatches as match}
        <div class="flex items-center justify-between">
          <span class="text-sm"
            ><strong>{match.firstName} {match.lastName}</strong>
            (#{match.shirtNumber}, {match.position})</span
          >
          <button
            type="button"
            onclick={() => confirmLink(match)}
            class="btn btn-sm preset-filled-primary-500 ml-3"
          >
            Ja, dat ben ik
          </button>
        </div>

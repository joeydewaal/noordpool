<script lang="ts">
	...
	function handleSubmit(e: Event) {
		e.preventDefault();
		updatePlayer(page.params.id, { name, shirtNumber, position, active });
...
{:else}
	<div class="max-w-lg">
		<a href="/players/{page.params.id}" class="text-sm text-primary hover:underline mb-4 inline-block">&larr; Terug naar speler</a>
		...
		<form onsubmit={handleSubmit} class="bg-white rounded-lg shadow p-6 space-y-4">
			...
			<div class="flex items-center gap-2">
				<input id="active" type="checkbox" bind:checked={active} class="rounded" />

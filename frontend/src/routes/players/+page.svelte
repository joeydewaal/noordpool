...
<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
    {#each filtered as player}
        <a
            ...
            class="bg-white rounded-lg shadow hover:shadow-md transition-shadow p-5 flex items-center gap-4 {!player.active
                ? 'opacity-60'
                : ''}"
        >
            ...
            <div class="flex-1 min-w-0">
                ...
                <span
                    class="inline-block mt-1 text-xs font-medium px-2 py-0.5 rounded-full {positionColor[
                        player.position
                    ]}"
                >
                    {player.position}
                ...
                {#if !player.active}
                    <span
                        class="inline-block mt-1 ml-1 text-xs font-medium px-2 py-0.5 rounded-full bg-gray-100 text-gray-500"
                    >
                        inactief
                    </span>
                {/if}

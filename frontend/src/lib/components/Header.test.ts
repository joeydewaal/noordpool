...
vi.mock('$lib/state/auth.svelte', () => ({
	auth: { isAuthenticated: false, isAdmin: false, isModerator: false, user: null },
}));
...
describe('Header today-game indicator', () => {
	it('shows no dot or subtitle when there is no game today', () => {
		...
		expect(screen.queryByLabelText('Wedstrijd vandaag')).not.toBeInTheDocument();
	...
	it('renders the "vs Opponent" subtitle in the desktop sidebar only', () => {
		...
		const subtitles = container.querySelectorAll('span.text-xs.text-surface-400');

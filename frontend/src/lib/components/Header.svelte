  ...
  >
    ...
    <Navigation.Footer class="mt-auto flex flex-col gap-2">
      ...
      {#if auth.isAuthenticated}
        ...
        {#if auth.isAdmin}
          <Navigation.TriggerAnchor href="/admin/users">
            <svg
              class="w-5 h-5"
              fill="none"
              stroke="currentColor"
            ...
            <Navigation.TriggerText>Gebruikers</Navigation.TriggerText>
          </Navigation.TriggerAnchor>
        {/if}
          ...
          {#if todaysGame}
            <span
              class="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-error-500 {isLiveToday
                ? 'animate-pulse'
                : ''}"
              aria-label={isLiveToday ? "Wedstrijd live" : "Wedstrijd vandaag"}
            ></span>
          {/if}
        </span>

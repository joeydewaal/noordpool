...
- Avatars use a presigned-URL flow: `POST /api/users/me/avatar/presign` returns `{ uploadUrl, publicUrl }`, the browser does a 256×256 WebP canvas resize and `PUT`s straight to `uploadUrl`, then `POST /api/users/me/avatar { url }` commits. Storage is pluggable via `r2::Backend` — `R2` uses `aws-sdk-s3` against Cloudflare R2 in prod (env: `R2_ACCOUNT_ID`, `R2_BUCKET`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_PUBLIC_URL`); `Local` writes to `AVATAR_DIR` and serves from `/avatars` for dev/tests, signing the upload URL with a HMAC-SHA256 token of the JWT secret.
